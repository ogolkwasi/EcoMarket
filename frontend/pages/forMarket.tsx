...
const tokenAccountAddress = " "
  ...
  async function loadNFTs() {
    const provider = new ethers.providers.JsonRpcProvider('https://arb-goerli.g.alchemy.com/v2/8vfEgr7DAq98NsHITlgJQCA4_Dj9mE2B');
  ...
  {nfts.map((nft, i) => (
    <div key={i} style={{ border: '3px solid #F8F6F4', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center' }}>
      ...
      <div className={styles.container_amounts_tons}> 
        <p>  <span style={{ color: "#F6F6F6", fontWeight: 'bold' }}>{tonsAmount(nft)}</span></p>
      </div>
      <div className={styles.container_amounts_tokens}>
        <p>  <span style={{ color: "#F6F6F6", fontWeight: 'bold' }}>{priceTons(nft)}</span></p>
      </div>
      ...
      <div className={styles.container_input_amounts_tokens}>
        ...
        <div className={styles.container_amounts_tokens}>
        <p>  <span style={{ color: "#F6FA70", fontWeight: 'bold' }}>{calculateTotalPrice(nft, tons)}</span></p>
      </div>
      &nbsp;
      &nbsp;
      &nbsp;
