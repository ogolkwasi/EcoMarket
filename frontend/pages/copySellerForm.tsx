...
export default function CarbonForm() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      //const fileName = formData.image.imageName; // Get the file name from the form data

    // Append the file name to the end of the CID
    //const image = `${imageCID}/${encodeURIComponent(fileName)}`;
      ...
      if (response.status === 200) {
        ...
        if (data && data.value && data.value.cid) {
          ...
          // After successfully uploading metadata, mint the NFT
          listNFT(cid, name, description, image);
        } else {
          console.error('Failed to extract CID from response:', data);
        }
      } else {
  ...
  const listNFT = async (metadataCID: any, name: string, description: object, image: object) => {
    ...
    try {
      ...
      // Replace with the appropriate function to mint an NFT
      const transaction = await contract.createNFT( name, description, image, price, { value: listingPrice });
